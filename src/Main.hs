...
libraryServer =
  resolver
    ...
    , union @"Writing"
        ( \case
            (ABook x) -> pure $ unionChoice @"Book" x
 ...
 where
  ...
  authorBooks aid =
    pure $
      maybe [] (map (ABook . (aid,) . fst) . thd3) (findBook aid)
  ...
  findBookTitle rx =
    pure $
      listToMaybe
        [ (aid, bid) | (aid, _, books) <- library, (bid, (title, _)) <- books, title =~ rx
        ]
